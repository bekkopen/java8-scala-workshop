...
class Ex2_LambdaOnCollectionsPart1Spec extends FlatSpec with Matchers {

	"teamNames" should "result in a list of the names of the teams" in {
		val teams = manchesterUnited :: liverpool :: arsenal :: Nil

		val teamNames = Ex2_LambdaOnCollectionsPart1.teamNames(teams)
	...
	"removeOldPlayers" should "remove players older than a given date" in {
		val result = Ex2_LambdaOnCollectionsPart1.removeOldPlayers(LocalDate.of(1984, 1, 1), bayernMunchen.players)
	...
	"addValueToEachTeam" should "add value in percentage to each team" in {
		...
		val teamsWithFivePercentValueIncrease = Ex2_LambdaOnCollectionsPart1.addValueToEachTeam(0.05, teams)
	...
	"removeTeamsWithLowValue" should "remove teams with value lower than limit" in {
		...
		val result = Ex2_LambdaOnCollectionsPart1.removeTeamsWithLowValue(teams, 1330000000.0)
	...
	"setLeagueNamesToUpperCase" should "uppercase league names" in {
		val result = Ex2_LambdaOnCollectionsPart1.setLeagueNamesToUpperCase(leagues)
